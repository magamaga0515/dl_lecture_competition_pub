...
from src.data_augmentation import EventDataAugmentation
...
class Sequence(Dataset):
    def __init__(self, seq_path: Path, representation_type: RepresentationType, mode: str = 'train', delta_t_ms: int = 100,
                 num_bins: int = 4, transforms=[], name_idx=0, visualize=False, load_gt=False, height=480, width=640):
        assert num_bins >= 1
        ...
        self.indices = np.arange(len(timestamps_flow) - 1)  # We need pairs of consecutive frames
        ...
        # Data augmentation
        self.augment = EventDataAugmentation(self.height, self.width)
    ...
    def load_flow(self, flowfile: Path) -> torch.Tensor:
    ...
    def get_data(self, index) -> Dict[str, Any]:
        ...
        ts_start_next = self.timestamps_flow[index + 1] - self.delta_t_us
        ts_end_next = self.timestamps_flow[index + 1]
        ...
        event_representation = self.augment(event_representation)
        event_representation_next = self.augment(event_representation_next)

        output['event_volume'] = torch.cat([event_representation, event_representation_next], dim=0)
        
        if self.load_gt and self.flow_png is not None:
            flow_file = self.flow_png[index]
            output['flow_gt'] = self.load_flow(flow_file)
...
def train_collate(sample_list):
    ...
    for field_name in sample_list[0]:
        ...
        if field_name.startswith("event_volume"):
            tensors = [torch.tensor(sample[field_name]) if isinstance(sample[field_name], np.ndarray) else sample[field_name] for sample in sample_list]
            batch[field_name] = torch.stack(tensors)
        if field_name.startswith("flow_gt"):
            if all(field_name in x for x in sample_list):
                tensors = [torch.tensor(sample[field_name][0]) if isinstance(sample[field_name][0], np.ndarray) else sample[field_name][0] for sample in sample_list]
                batch[field_name] = torch.stack(tensors)
                tensors_valid_mask = [torch.tensor(sample[field_name][1]) if isinstance(sample[field_name][1], np.ndarray) else sample[field_name][1] for sample in sample_list]
                batch[field_name + '_valid_mask'] = torch.stack(tensors_valid_mask)
